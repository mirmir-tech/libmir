...
fn execute(
    ...
    let native =
        loaded.prefill_pooled_vision(session_id, prompt, image, execution_sampling, progress)?;
    ...
    let trace = format!(
        "native pooled vision prefill: {} tokens, prefix cache disabled, {} runtime KV blocks, {} cached tokens, {:.3}ms",
    ...
    tracing::debug!(
        ...
        cached_tokens,
        "native pooled vision Metal prefill completed"
    );
    Ok(PrefillOutput {
