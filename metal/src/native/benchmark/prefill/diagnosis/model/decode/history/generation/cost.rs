...
fn compares_qwen_generation_reservation() -> Result<()> {
    ...
    for (run, plan) in [a, b, a, b, b, a, b, a, a, b].into_iter().enumerate() {
        ...
        writeln!(
            ...
            "generation.model: {}",
            json!({
                "started_epoch_ms":started,"run":run,"plan":plan,"warmup":run<2,"matches_reference":matches,"observation":observation,
            })
        )?;
        assert!(matches);
        if run >= 2 {
