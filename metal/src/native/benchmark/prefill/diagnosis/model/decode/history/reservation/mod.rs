...
fn horizon() -> Result<DecodeReservation> {
    Ok(DecodeReservation::Tokens(
...
fn inspects_qwen_decode_reservation() -> Result<()> {
    inspect(2049, 30)
}

#[test]
#[ignore = "GPT-OSS reserved decode tail allocation/numerical gate; set MIRMIR_BENCH_MODEL"]
fn inspects_clamped_decode_reservation() -> Result<()> {
    inspect(129, 36)
}
