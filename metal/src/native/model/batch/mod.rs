...
mod plan;
#[cfg(test)]
pub(in crate::native) mod profile;
#[cfg(test)]
mod tests;
pub(in crate::native) use plan::DecodeExecution;
...
impl LoadedModel {
    ...
    pub(crate) fn can_decode_batch(&self, inputs: &[DecodeInput]) -> bool {
        inputs.len() > 1
            ...
            && inputs.iter().all(|input| self.can_decode_packed_row(input))
    }
    ...
    fn decode_batch_admitted(
        &mut self,
        inputs: &[DecodeInput],
        #[cfg(test)] profile: Option<&mut profile::Profile>,
    ) -> Result<Vec<NativeOutput>> {
        let mut states = self.take_batch_states(inputs)?;
        let result = self.execution.decoder().and_then(|model| {
            decode_states(
                model,
                &self.stream,
                inputs,
    ...
    fn validate_decode_inputs(&self, inputs: &[DecodeInput]) -> Result<()> {
...
fn decode_states(
    ...
    #[cfg(test)] mut profile: Option<&mut profile::Profile>,
) -> Result<Vec<NativeOutput>> {
    let token_ids = sample_batch(inputs, states, stream)?;
    token_ids.async_eval(stream)?;
    #[cfg(test)]
    if let Some(profile) = profile.as_deref_mut() {
        profile.mark(profile::Stage::SampleSubmit);
    }
    ...
    #[cfg(test)]
    if let Some(profile) = profile.as_deref_mut() {
        profile.mark(profile::Stage::GraphBuild);
    }
    ...
    #[cfg(test)]
    if let Some(profile) = profile.as_deref_mut() {
        profile.mark(profile::Stage::RootsSubmit);
    }
    let tokens = token_ids.to_vec_u32(stream)?;
    #[cfg(test)]
    if let Some(profile) = profile.as_deref_mut() {
        profile.mark(profile::Stage::TokenRead);
    }
    if tokens.len() != states.len() {
        return Err(Error::InvalidDecodeBatch("sampled token count does not match rows".into()));
    }
    let outputs = complete_batch(&logits, tokens, stream, states)?;
    #[cfg(test)]
    if let Some(profile) = profile {
        profile.mark(profile::Stage::Complete);
    }
    Ok(outputs)
}
