...
#[cfg(test)]
pub mod probe;
pub(in crate::engine) mod profile;
...
pub trait LoweredPackedLayer: LoweredLayer {
...
pub fn forward_layers<L: LoweredLayer>(
    ...
    for (index, layer) in layers.iter().enumerate() {
        ...
        #[cfg(test)]
        probe::record(index, probe::Stage::Input, &hidden)?;
        hidden = layer.forward_mixer(&hidden, cache, index, context)?;
        #[cfg(test)]
        probe::record(index, probe::Stage::Mixer, &hidden)?;
        hidden = layer.forward_feed_forward(&hidden, context)?;
        #[cfg(test)]
        probe::record(index, probe::Stage::FeedForward, &hidden)?;
...
pub fn forward_packed_layers<L: LoweredPackedLayer>(
    ...
    let profiling = stream.config().diagnostics.profile_components;
    for (index, layer) in layers.iter().enumerate() {
        ...
        profile::record(
            ...
            stream,
            index,
            profile::Component::Mixer(layer.mixer_kind()),
        )?;
        let started = profiling.then(Instant::now);
        hidden = layer.forward_packed_feed_forward(&hidden, caches.len(), stream)?;
        #[cfg(test)]
        probe::record(index, probe::Stage::FeedForward, &hidden)?;
        profile::record(started, &hidden, caches, stream, index, profile::Component::FeedForward)?;
    }
    Ok(hidden)
}
