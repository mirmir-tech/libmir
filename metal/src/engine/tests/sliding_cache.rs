...
fn updates_sliding_cache_with_a_chunk_across_the_window() -> Result<()> {
    ...
    assert_eq!(context.keys.to_vec_f32()?, vec![2.0, 3.0, 4.0, 5.0]);
    let mask = context.mask.as_ref().ok_or(crate::engine::Error::NullHandle("sliding mask"))?;
    assert_eq!(mask.to_vec_u32_on_stream(&stream)?, vec![1, 1, 1, 0, 0, 1, 1, 1]);
    ...
    assert_eq!(output.to_vec_f32_on_stream(&stream)?, vec![3.0, 4.0]);

    let token = Array::from_f32(&[6.0], &[1, 1, 1, 1])?;
    let context = cache.update(&token, &token, &stream)?;
    assert_eq!(context.keys.to_vec_f32_on_stream(&stream)?, vec![4.0, 5.0, 6.0]);
    Ok(())
}
...
fn preserves_temporal_order_across_consecutive_sliding_chunks() -> Result<()> {
    ...
    assert_eq!(context.keys.to_vec_f32_on_stream(&stream)?, vec![4.0, 5.0, 6.0, 7.0]);
    let mask = context.mask.as_ref().ok_or(crate::engine::Error::NullHandle("sliding mask"))?;
    assert_eq!(mask.to_vec_u32_on_stream(&stream)?, vec![1, 1, 1, 0, 0, 1, 1, 1]);
    Ok(())
}
