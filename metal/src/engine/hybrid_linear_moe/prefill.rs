    ...
    pub fn forward_multimodal_prefill(
        ...
        if token_shape.len() != 2 || token_shape[0] != 1 {
            return Err(Error::InvalidModel(format!(
                "Qwen multimodal token IDs must have shape [1, sequence], got {token_shape:?}"
            )));
        }
        ...
        if image_embeddings.shape()? != expected_image {
            return Err(Error::InvalidModel(format!(
                "Qwen image embeddings must have shape {expected_image:?}"
            )));
        }
        if position_ids.shape()? != [3, i32::try_from(sequence)?] {
            return Err(Error::InvalidModel(
                "Qwen multimodal position IDs must have shape [3, sequence]".into(),
            ));
        }
