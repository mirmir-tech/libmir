...
pub(super) fn load(path: &Path) -> Option<StoredProfile> {
    // Initialize identity during tuner construction even on a cache miss, so
    // the first persisted decision cannot hash the executable during decode.
    let environment = Environment::current().ok()?;
