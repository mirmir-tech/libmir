// Test comparator: production recurrence with explicit ascending butterfly reductions.
...
projection += simd_shuffle_xor(projection, 4);
projection += simd_shuffle_xor(projection, 8);
projection += simd_shuffle_xor(projection, 16);
...
result += simd_shuffle_xor(result, 8);
result += simd_shuffle_xor(result, 16);
