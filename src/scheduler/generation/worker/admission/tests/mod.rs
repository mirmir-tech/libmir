...
use super::{
    completion_wave_rows, next_prefill_deadline, pending_prefill_tokens, prefill_quiet_wait,
    prefill_wave_limit, resident_wave_rows, take_ready,
};
...
fn idle_prefill_collects_a_short_burst() {
    assert_eq!(prefill_quiet_wait(200, 1, 16), Duration::from_millis(30));
    assert_eq!(prefill_quiet_wait(200, 16, 16), Duration::ZERO);
    assert_eq!(prefill_quiet_wait(0, 1, 16), Duration::ZERO);
}
