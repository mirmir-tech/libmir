...
pub(super) fn logical(spec: &SemanticModelSpec, role: &LogicalTensorRole) -> Option<Vec<usize>> {
    match role {
...
pub(super) fn transforms(
    ...
    if matches!(storage, TensorStorage::Dense { .. })
        && logical(spec, role).is_some_and(|logical| {
            logical.len() == 2 && physical == [logical[1], logical[0]] && physical != logical
        })
    {
...
fn experts(
    ...
    let input = match projection {
        ExpertProjectionRole::Gate | ExpertProjectionRole::Up | ExpertProjectionRole::GateUp => {
            hidden
        },
        ExpertProjectionRole::Down => routed.intermediate_size,
    };
    match expert {
...
fn routed(feed_forward: &FeedForwardSpec) -> Option<&crate::semantic::RoutedExpertsSpec> {
    match feed_forward {
...
fn shared(feed_forward: &FeedForwardSpec) -> Option<&crate::semantic::SharedExpertSpec> {
    match feed_forward {
