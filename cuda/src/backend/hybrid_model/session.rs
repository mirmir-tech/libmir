...
use super::{CudaHybridLinearLayerState, CudaHybridLinearModelTemplate, plan::HybridExecutionPlan};
use crate::{
    CudaAffineOutputHead, CudaBackend, DeviceSamplerBf16, Error, Result,
    kernels::{SelectRowBf16, ShiftedRmsNorm},
};

/// One mutable CUDA session for an affine hybrid linear/full-attention model.
pub struct CudaHybridLinearModelSession {
    template: CudaHybridLinearModelTemplate,
    embedding: crate::AffineQuantizedEmbedding,
    states: Vec<CudaHybridLinearLayerState>,
    plans: HashMap<usize, HybridExecutionPlan>,
...
impl CudaHybridLinearModelSession {
    pub(super) fn new(template: &CudaHybridLinearModelTemplate) -> Result<Self> {
    ...
    fn prefill_inner(
        ...
        if !self.plans.contains_key(&count) {
            self.plans.insert(count, HybridExecutionPlan::new(&self.template, count)?);
        }
        let plan = self
            .plans
            .get_mut(&count)
            .ok_or(Error::InvalidDecoderKernel("missing hybrid execution plan"))?;
        ...
        self.position = self
            .position
            .checked_add(count)
            .ok_or(Error::InvalidDecoderKernel("hybrid session position overflow"))?;
        Ok(&self.logits)
    }
    ...
    fn validate(&self, tokens: &[u32], positions: &[u32], table: &BlockTable) -> Result<()> {
        let end = self
            ...
            .ok_or(Error::InvalidPagedKv("hybrid prompt range overflow"))?;
        if tokens.is_empty() || positions.len() != 3 * tokens.len() || table.token_len() != end {
            return Err(Error::InvalidPagedKv("hybrid prompt differs from session position"));
        }
        Ok(())
    }
}
