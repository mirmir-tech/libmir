...
use super::MxFp8GatheredMoeBf16;
use crate::{
    CudaBackend, ExecutionPhase, GatedActivation, PlanSource, Result,
    backend::{
        linear::mxfp8::MxFp8ExpertWeights,
        tuning::{MoeProfileExecution, MoeProfileRequest, MxFp8MoeExecution},
    },
};
...
pub(super) fn prepare(
    ...
    if let Some((MoeProfileExecution::MxFp8(execution), source)) =
        backend.auto_tuner().lookup_moe(request)
    {
        trace(request, execution, source, None);
        return MxFp8GatheredMoeBf16::with_candidates(
            backend,
            tokens,
            selected_count,
            activation,
    ...
    let fallback = MxFp8MoeExecution::EightWarps;
    ...
    match tune(backend, request, tokens, selected_count, activation, weights) {
        ...
        Err(error) => {
            ...
            tracing::warn!(?request, %error, "CUDA gathered MXFP8 tuning failed");
            MxFp8GatheredMoeBf16::with_candidates(
                backend,
                tokens,
                selected_count,
                activation,
...
fn tune(
    ...
    let executions = [MxFp8MoeExecution::EightWarps, MxFp8MoeExecution::FourWarps];
    let mut plan = MxFp8GatheredMoeBf16::with_candidates(
    ...
    let fastest = timings
        ...
        .map(|(index, _)| index)
        .ok_or(crate::Error::InvalidExecutionPlan("MXFP8 tuner has no candidates"))?;
    ...
    backend.auto_tuner().record_moe(
        request,
        MoeProfileExecution::MxFp8(execution),
        average,
        started.elapsed(),
    );
...
fn validate(
    ...
    for index in 1..plan.candidates.len() {
        ...
        if read(backend, output)? != expected {
            return Err(crate::Error::InvalidExecutionPlan("MXFP8 candidates differ"));
        }
    }
    Ok(())
}
...
fn trace(
    ...
    tracing::info!(
        ...
        "selected CUDA gathered MXFP8 MoE execution"
    );
}
