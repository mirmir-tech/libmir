...
use super::{MxFp4ExpertWeights, MxFp4GatheredBf16Linear, weights::MxFp4GateUpWeights};
...
#[derive(Debug)]
struct Candidate {
    execution: crate::backend::tuning::MxFp4MoeExecution,
    gate_up: CandidateGateUp,
    down: MxFp4GatheredBf16Linear,
}

#[derive(Debug)]
enum CandidateGateUp {
    Separate {
        gate: MxFp4GatheredBf16Linear,
        up: MxFp4GatheredBf16Linear,
    },
    Interleaved {
        gate_up: MxFp4GatheredBf16Linear,
    },
}
...
#[derive(Debug)]
/// Autotuned gathered MXFP4 routed MLP with accelerator-resident routing.
pub struct MxFp4GatheredMoeBf16 {
...
impl MxFp4GatheredMoeBf16 {
    ...
    fn with_candidates(
        ...
        activation: GatedActivation,
        weights: &MxFp4ExpertWeights,
        executions: &[crate::backend::tuning::MxFp4MoeExecution],
    ) -> Result<Self> {
        let assignments = tokens
            .checked_mul(selected_count)
            .ok_or(Error::InvalidDecoderKernel("MXFP4 expert assignment size overflow"))?;
        ...
        let gate_up_output =
            match weights.gate_up {
                MxFp4GateUpWeights::Separate { .. } => GateUpScratch::Separate {
                    gate: allocate(intermediate_elements)?,
                    up: allocate(intermediate_elements)?,
                },
                MxFp4GateUpWeights::Interleaved { .. } => GateUpScratch::Interleaved {
                    gate_up: allocate(intermediate_elements.checked_mul(2).ok_or(
                        Error::InvalidDecoderKernel("MXFP4 fused gate/up size overflow"),
                    )?)?,
                },
            };
        Ok(Self {
    ...
    fn execute_candidate(
        ...
        match (&candidate.gate_up, &weights.gate_up, &mut self.gate_up_output) {
            (
                ...
                MxFp4GateUpWeights::Separate { gate: gate_weight, up: up_weight },
            ...
            (
                ...
                MxFp4GateUpWeights::Interleaved { gate_up: weight },
            ...
            _ => return Err(Error::InvalidExecutionPlan("MXFP4 gate/up storage changed")),
        }
...
impl Candidate {
    fn new(
        ...
        let gate_up = match &weights.gate_up {
            MxFp4GateUpWeights::Separate { gate, up } => CandidateGateUp::Separate {
            ...
            MxFp4GateUpWeights::Interleaved { gate_up } => CandidateGateUp::Interleaved {
                gate_up: gate_up
