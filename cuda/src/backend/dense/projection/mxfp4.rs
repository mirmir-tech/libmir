...
use crate::{
    CudaBackend, CudaTensor, DenseGateUpWeights, Error, MxFp4Bf16Linear, Result, RmsNormBf16,
};
...
) -> Result<Box<[MxFp4Bf16Linear; 2]>> {
    let DenseGateUpWeights::MxFp4 { gate, up } =
        weights.ok_or(Error::InvalidExecutionPlan("MXFP4 MLP requires gate/up weights"))?
    else {
        return Err(Error::InvalidExecutionPlan("MXFP4 MLP received other gate/up weights"));
    };
    Ok(Box::new([gate.prepare(backend, tokens)?, up.prepare(backend, tokens)?]))
}

pub(super) fn execute_gate_up(
    operations: &[MxFp4Bf16Linear; 2],
    ...
    let DenseGateUpWeights::MxFp4 { gate, up } = weights else {
        return Err(Error::InvalidExecutionPlan("MXFP4 gate/up operation received other weights"));
    };
