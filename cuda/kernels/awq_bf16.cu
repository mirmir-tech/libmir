...
__device__ __forceinline__ float awq_bf16_to_float(unsigned short value) {
  return __uint_as_float(static_cast<unsigned int>(value) << 16u);
}

__device__ __forceinline__ unsigned short awq_float_to_bf16(float value) {
  const unsigned int bits = __float_as_uint(value);
  const unsigned int rounding = 0x7fffu + ((bits >> 16u) & 1u);
  return static_cast<unsigned short>((bits + rounding) >> 16u);
}

__device__ __forceinline__ unsigned int awq_nibble_shift(unsigned int row) {
  const unsigned int lane = row & 7u;
  return (((lane & 1u) << 2u) | (lane >> 1u)) * 4u;
}

__device__ __forceinline__ float awq_value(
    const int* weight, const int* zero_points, const unsigned short* scales,
    unsigned int row, unsigned int feature, unsigned int output_features,
    unsigned int group_size) {
  const unsigned int packed_output = output_features / 8u;
  const unsigned int shift = awq_nibble_shift(row);
  const unsigned int weight_word = static_cast<unsigned int>(
      weight[feature * packed_output + row / 8u]);
  const unsigned int group = feature / group_size;
  const unsigned int zero_word = static_cast<unsigned int>(
      zero_points[group * packed_output + row / 8u]);
  const unsigned int value = (weight_word >> shift) & 15u;
  const unsigned int zero = (zero_word >> shift) & 15u;
...
extern "C" __global__ void libmir_cuda_awq_gemv_bf16(
    const unsigned short* input, const int* weight, const int* zero_points,
    const unsigned short* scales, unsigned short* output,
...
#pragma unroll
    for (unsigned int offset = 0; offset < values_per_thread; ++offset) {
      ...
      if (feature < input_features) {
        sum += awq_bf16_to_float(input[feature]) *
            awq_value(
                weight, zero_points, scales, row, feature, output_features,
                group_size);
      }
    }
  }
  ...
  if (threadIdx.x == 0) {
    output[row] = awq_float_to_bf16(sum);
  }
}

extern "C" __global__ void libmir_cuda_awq_qmm_bf16(
    const unsigned short* input, const int* weight, const int* zero_points,
    const unsigned short* scales, unsigned short* output, unsigned int tokens,
    unsigned int input_features, unsigned int output_features,
    unsigned int group_size) {
  ...
  wmma::store_matrix_sync(
      output_tiles[warp], accumulator, tile, wmma::mem_row_major);
  for (unsigned int index = lane; index < tile * tile; index += 32u) {
    const unsigned int token = token_base + index / tile;
    const unsigned int row = output_base + index % tile;
