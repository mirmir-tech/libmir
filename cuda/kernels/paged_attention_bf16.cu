...
extern "C" __global__ void libmir_cuda_paged_attention_bf16(
    const __nv_bfloat16* query, const unsigned char* key_pages,
    const unsigned char* value_pages, const unsigned int* block_table,
    __nv_bfloat16* output, unsigned int token_count,
    unsigned int block_count, unsigned int block_size,
  ...
  if (query_head >= query_heads || token_count == 0 ||
      (split_threshold > 0 && visible_tokens >= split_threshold)) return;
  ...
  for (unsigned int token = first; token < token_count; ++token) {
    ...
    const unsigned int slot = token % block_size;
    const unsigned int page_token = physical_block * block_size + slot;
    float dot = 0.0f;
  ...
  for (unsigned int dimension = lane; dimension < value_head_dim;
    ...
    output[query_head * value_head_dim + dimension] =
        __float2bfloat16_rn(accumulators[local] / denominator);
  }
}
